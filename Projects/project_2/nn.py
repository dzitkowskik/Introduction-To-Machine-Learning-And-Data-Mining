...
def main():
    ...
    n_units = [17, 26, 1]     # number of layer units
    ...
    for train_index, test_index in CV:
        ...
        map_values = (lambda x: int(round((x + 1.0) * 12.5)))
        result = map(map_values, ravel(bestnet[k].sim(X_test)))
        y_est = le.inverse_transform(result)
        e = (ravel(y_est) != ravel(y_test)).sum().astype(float)/y_test.shape[0]
        print e
        bestnet[k].save('bestnet_' + str(e) + '.net')
        errors[k] = e
        k += 1
