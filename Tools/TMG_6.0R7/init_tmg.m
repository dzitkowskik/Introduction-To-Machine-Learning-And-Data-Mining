...
if exist('mysql_info.mat'), 
    ...
    if isfield(mysql, 'run'), 
        ...
        if res~=1, 
            if mysql.is_set==1, an=new_sprintf('WARNING: Unable to create MySQL database! '); disp(an); end
            mysql.is_set=0;
        else, 
            an=new_sprintf('Database created successfully!'); disp(an); 
        end        
...
disp(an1); disp(an1); an=new_sprintf('You are about to installed the TMG Toolbox to you system. Press any key to continue!'); disp(an);disp(an1); disp(an1); pause;
...
if tmp==1, 
    ...
    if mysql.is_set==1, 
        ...
        if quit_matlab~=1, an=new_sprintf('Installation has not been completed! Please run init_tmg again! '); disp(an); return; end
        mysql.run=1;
    end
else, 
    mysql.is_set=0;
...
disp(an1); an=new_sprintf('Adding directories to MATLAB path...'); disp(an);disp(an1); 
str=strcat(fileparts(mfilename('fullpath'))); addpath(str); 
an=new_sprintf('Adding directory %s...', str); disp(an);
...
an=new_sprintf('Adding directory %s...', str); disp(an);
str=strcat(fileparts(mfilename('fullpath')), filesep, 'filters'); addpath(str); 
an=new_sprintf('Adding directory %s...', str); disp(an);
str=strcat(fileparts(mfilename('fullpath')), filesep, 'log_files'); addpath(str); 
an=new_sprintf('Adding directory %s...', str); disp(an);
...
if ~isempty(conn.Message), an=new_sprintf('MySQL: Unknown user or wrong password... Skipping...'); disp(an);res=-1; return; end
