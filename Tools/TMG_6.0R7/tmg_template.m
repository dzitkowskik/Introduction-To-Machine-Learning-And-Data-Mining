...
OPTIONS.dsp=1;
OPTIONS.remove_al=0;
OPTIONS.remove_num=0;
OPTIONS.parse_subd=0;
% --------------------------------------------------------------------

% ---Run tmg----------------------------------------------------------
filename=strcat(fileparts(mfilename('fullpath')),filesep,'sample_documents/sample1');
[A, dictionary, global_weights, normalization_factors, words_per_doc, titles, files, update_struct]=...
    tmg(filename, OPTIONS);
% --------------------------------------------------------------------
...
% ---Run tmg_query----------------------------------------------------------
query=strcat(fileparts(mfilename('fullpath')),filesep,'sample_documents/sample_query1');
[Q, words_per_query, titles_query, files_query] = ...
    tmg_query(query, dictionary, OPTIONS);
% --------------------------------------------------------------------------
