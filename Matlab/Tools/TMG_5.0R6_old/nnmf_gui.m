...
if get(handles.BLOCKNNDSVDRadio, 'Value')==1 | get(handles.CLRadio, 'Value')==1, 
    ...
    if ~isequal(clust_method, 'pddp'), 
        ...
        if state == 1, 
            ...
            if tmp>k,
                err_pddp=sprintf('Number of principal components for PDDP you supplied produces more than %d clusters (number of NNMF factors). Please alter your selection...', k); 
...
if isequal(view_res, 'Yes'), 
    if get(handles.RefineFactorsCheckbox, 'Value') == 1, 
        if get(handles.refinementMethod, 'Value') == 1, 
            title = sprintf('Multiplicative update %s', title);
        else, 
            title = sprintf('ANLS %s', title);
        end
    end
    if get(handles.RandomRadio, 'Value')==1, 
