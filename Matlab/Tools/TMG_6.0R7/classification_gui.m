...
OPTIONS=struct('delimiter', 'none_delimiter', 'line_delimiter', 1, 'stemming', 0, ...
        'local_weight', 't', 'dsp', 0, 'parse_subd', 0, 'remove_al', 0, 'remove_num', 0);
...
handles.objects=[handles.Labels;handles.LabelsButton;handles.StoredLabels;handles.SQuery;...
    ...
    handles.LineDelimiter;handles.RemoveNum;handles.RemoveAl;handles.ParseSubd;];
...
set(handles.RemoveAl, 'Visible', 'off');  set(handles.RemoveNum, 'Visible', 'off');
set(handles.ParseSubd, 'Visible', 'off');
...
if isempty(str), 
    an=new_sprintf('Select a dataset...'); 
...
if get(handles.StoredLabels, 'Value')==1, 
    try, 
        load(strcat(str, filesep, 'labels.mat')); 
         ...
         an=new_sprintf('Variable %s not found...', strcat(str, filesep, 'labels.mat')); 
        msgbox(an, 'Error', 'modal');
        return;             
    end
else, 
    if isempty(get(handles.Labels, 'String')), 
        an=new_sprintf('You didn''t supply the labels file-variable...'); 
        msgbox(an, 'Error', 'modal');
        return;
    end    
    if isequal(handles.labels_type, 'Files'), 
        try, 
            ...
            an=new_sprintf('Bad format for labels file %s...', get(handles.Labels, 'String')); 
            msgbox(an, 'Error', 'modal');
            return;             
        end        
    else, 
...
if ~iscell(labels),
        an=new_sprintf('Labels must be a cell array...'); 
	msgbox(an, 'Error', 'modal');
	return;                 
end

if size(A, 2)~=length(labels) | (size(labels, 2)~=1 & size(labels, 2)~=1 & size(A, 2)~=1),
    an=new_sprintf('Labels variable size does not match with number of documents...', get(handles.Labels, 'String')); 
	msgbox(an, 'Error', 'modal');
	return;                 
end
...
if iscell(labels)==1 & get(handles.SingleRadio, 'Value')==1, 
    an=new_sprintf('You supplied a multi-label variable for a single-label collection...', get(handles.Labels, 'String')); 
	msgbox(an, 'Error', 'modal');
	return;                     
end
...
if get(handles.MultiRadio, 'Value')==1, 
    if get(handles.UseThresholds, 'Value')==1, 
        if isempty(get(handles.Thresholds, 'String')), 
            an=new_sprintf('You haven''t supplied the thresholds variable...', get(handles.Labels, 'String')); 
        	msgbox(an, 'Error', 'modal');
        	return;                             
        end
        try, 
            ...
            an=new_sprintf('Variable %s not found...', get(handles.Thresholds, 'String')); 
        	msgbox(an, 'Error', 'modal');
        	return;                                         
        end
        if length(thresholds)~=length(unique_labels), 
            an=new_sprintf('Length of thresholds variable does not match number of classes...'); 
        	msgbox(an, 'Error', 'modal');
        	return;                                                                 
        end
    else, 
        if isempty(get(handles.Fvalue, 'String')), 
            an=new_sprintf('You haven''t supply the minimum F-value...'); 
        	msgbox(an, 'Error', 'modal');
        	return;                                                     
        end        
        minF1=str2double(get(handles.Fvalue, 'String'));
        if minF1<=0 | minF1>=1, 
            an=new_sprintf('Minimum F-value must be greater than 0 and less than 1...'); 
        	msgbox(an, 'Error', 'modal');
        	return;                                                     
        end
        thresholds=[];
...
if get(handles.SQueryRadio, 'Value')==1, 
    if isempty(get(handles.SQuery, 'String')), 
        an=new_sprintf('You didn''t provide a query...'); 
        msgbox(an, 'Error', 'modal');
        return;
    end
else, 
    if isempty(handles.filename), 
        an=new_sprintf('You didin''t supply a file...'); 
        msgbox(an, 'Error', 'modal');
        return;
    end
    if ~exist(handles.filename), 
        an=new_sprintf('File %s not found...', handles.filename); 
        msgbox(an, 'Error', 'modal');
        return;
    end
end
...
try, 
	...
	an=new_sprintf('Dictionary not found in %s...', strcat(str, filesep, 'dictionary.mat')); 
	msgbox(an, 'Error', 'modal');
	return;             
end        
if get(handles.FQueryRadio, 'Value')==1, 
    Q=tmg_query(handles.filename, dictionary,handles.OPTIONS); 
else, 
    ...
    Q=tmg_query(fname, dictionary, handles.OPTIONS); 
    delete(fname);
end
...
if get(handles.LLSFRadio, 'Value')==1, 
    ...
    if isempty(get(handles.nFactorsLLSF, 'String')), 
        an=new_sprintf('You haven''t supply the number of factors...'); 
        msgbox(an, 'Error', 'modal');
        return;        
    end
    l=str2double(get(handles.nFactorsLLSF, 'String')); 
    if l<k_max, 
        an=new_sprintf('Number of factors must be at least equal to the number of classes...'); 
        msgbox(an, 'Error', 'modal');
        return;                
    end
    if get(handles.MultiRadio, 'Value')==1, 
...
%             for j=1:length(tmp), 
%                 if j==1, tmp1=unique_labels{tmp(j)}; else, tmp1=new_sprintf('%s %s', tmp1, unique_labels{tmp(j)}); end
%             end
%             labels_as{i}=tmp1;
%         end
    else, 
...
%        labels_as={unique_labels{labels_as}}';
    ...
    if get(handles.kNNRadio, 'Value')==1, 
        if isempty(get(handles.kNN, 'String')), 
            an=new_sprintf('You haven''t supply the number of nearest neighboors...'); 
            msgbox(an, 'Error', 'modal');
            return;        
        end
        knn=str2double(get(handles.kNN, 'String')); 
        if knn<1, 
            an=new_sprintf('Number of nearest neighboors must be at least 1...');
            msgbox(an, 'Error', 'modal');
            return;                
        end            
    end
    if get(handles.RocchioRadio, 'Value')==1, 
        if isempty(get(handles.RocchioPos, 'String')), 
            an=new_sprintf('You haven''t supply the weight of positive examples...'); 
            msgbox(an, 'Error', 'modal');
            return;        
        end
        r_pos=str2double(get(handles.RocchioPos, 'String')); 
        if r_pos<=0, 
            an=new_sprintf('Positive weight must be a positive value...'); 
            msgbox(an, 'Error', 'modal');
            return;                
        end            
        if isempty(get(handles.RocchioNeg, 'String')), 
            ...
            if r_neg<0, 
                an=new_sprintf('Negative weight must be a non-negative value...'); 
                msgbox(an, 'Error', 'modal');
                return;                set(handles.RemoveAl, 'Visible', 'off');set(handles.RemoveNum, 'Visible', 'off');set(handles.ParseSubd, 'Visible', 'off');
            end                        
        end
    end        
    if get(handles.kNNRadio, 'Value')==1, 
...
set(handles.RemoveAl, 'Visible', 'off'); set(handles.RemoveNum, 'Visible', 'off'); set(handles.ParseSubd, 'Visible', 'off');
...
function SQueryRadio_Callback(hObject, eventdata, handles)%Single Query
...
function FQueryRadio_Callback(hObject, eventdata, handles) %File Query Button
...
set(handles.RemoveNum, 'Visible', 'on');set(handles.RemoveAl, 'Visible', 'on');set(handles.ParseSubd, 'Visible', 'on');
set(handles.RemoveNum, 'Enable', 'on');set(handles.RemoveAl, 'Enable', 'on');set(handles.ParseSubd, 'Enable', 'on');
handles.OPTIONS.delimiter='none_delimiter';
%states=zeros(39, 1);states([(35:39)])=1;
guidata(hObject, handles);
...
s=get(hObject, 'String');
if ~isempty(s), 
	 an=new_sprintf('Use the Browse button in order to set the thresholds variable...'); 
	msgbox(an, 'Error', 'modal');
    set(hObject, 'String', '');
	return;
end
...
% --- Executes on button press in RemoveNum.
function RemoveNum_Callback(hObject, eventdata, handles)
% hObject    handle to RemoveNum (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
OPT=handles.OPTIONS;
s=get(hObject, 'Value');
if s==1, OPT.remove_num=1; else, OPT.remove_num=0; end
handles.OPTIONS=OPT;
guidata(hObject, handles);
% Hint: get(hObject,'Value') returns toggle state of RemoveNum


% --- Executes on button press in RemoveAl.
function RemoveAl_Callback(hObject, eventdata, handles)
% hObject    handle to RemoveAl (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
OPT=handles.OPTIONS;
s=get(hObject, 'Value');
if s==1, OPT.remove_al=1; else, OPT.remove_al=0; end
handles.OPTIONS=OPT;
guidata(hObject, handles);
% Hint: get(hObject,'Value') returns toggle state of RemoveAl


% --- Executes on button press in ParseSubd.
function ParseSubd_Callback(hObject, eventdata, handles)
% hObject    handle to ParseSubd (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
OPT=handles.OPTIONS;
s=get(hObject, 'Value');
if s==1, OPT.parse_subd=1; else, OPT.parse_subd=0; end
handles.OPTIONS=OPT;
guidata(hObject, handles);
% Hint: get(hObject,'Value') returns toggle state of ParseSubd
