...
OPTIONS=struct('use_mysql', 0, 'delimiter', 'emptyline', 'line_delimiter', 1, 'stemming', 0, 'min_length', 3, 'max_length', 30, ...
        ...
        'local_weight', 't', 'global_weight', 'x', 'normalization', 'x', 'dsp', 1, 'remove_num', 0, 'remove_al', 0,'parse_subd',0);
...
handles.objects=[handles.Filename;handles.FilenameButton;handles.Dictionary;handles.DictionaryButton;handles.GWQuery;...
        ...
        handles.StoreIn;handles.RemoveNum;handles.RemoveAl;handles.ParseSubd;];
...
if create_flag,
    ...
    if isfield(OPT, 'db_name'), 
        if exist(strcat(fileparts(mfilename('fullpath')), filesep, 'data', filesep, OPT.db_name)), 
           an=new_sprintf('%s collection already exists. Please give another name...', OPT.db_name);msgbox(an, 'Error', 'modal'); return;
        end
    end
...
if create_query_flag,
    ...
    if isequal(handles.dictionary_type, 'Files'),
        if exist(handles.dictionary)==0,
            an=new_sprintf('File %s not found...', handles.dictionary);
    ...
    OPTS1.local_weight=OPT.local_weight;OPTS1.stemming=OPT.stemming;OPTS1.dsp=OPT.dsp;OPTS1.remove_num=OPT.remove_num;OPTS1.remove_al=OPT.remove_al;OPTS1.parse_subd=OPT.parse_subd;

    if isfield(OPT, 'global_weights'),
        s=OPT.global_weights;
        if isequal(handles.gwquery_type, 'Files'),
            if ~isempty(s) & exist(s)==0,
                an=new_sprintf('File %s not found...', s);
                msgbox(an, 'Error', 'modal');
                return;
            else,
                load(s);
...
if update_flag,
    ...
    if isequal(handles.update_struct_type, 'Files'),
        if exist(handles.update_struct)==0,
            an=new_sprintf('File %s not found...', handles.update_struct);
            msgbox(an, 'Error', 'modal');
            return;
        else,
    ...
    OPTS1.line_delimiter=OPT.line_delimiter;
    OPTS1.remove_num=OPT.remove_num;
    OPTS1.remove_al=OPT.remove_al;
    OPTS1.parse_subd=OPT.parse_subd;
...
if downdate_flag,
    ...
    if isequal(handles.removed_docs_type, 'Files'),
        if exist(handles.removed_docs)==0,
            an=new_sprintf('File %s not found...', handles.removed_docs);
            msgbox(an, 'Error', 'modal');
            return;
        else,
...
if create_query_flag==0,
    ...
    if create_flag==0, 
       an=new_sprintf('Do you want to save the results?');
...
OPTIONS=struct('delimiter', 'emptyline','line_delimiter', 1, 'stemming', 0, 'min_length', 3, 'max_length', 30, ...
        'min_local_freq', 1, 'min_global_freq', 1, 'max_local_freq', inf, 'max_global_freq', inf, ...
        'local_weight', 't', 'global_weight', 'x', 'normalization', 'x', 'dsp', 1, 'remove_num', 0, 'remove_al', 0, 'parse_subd',0);
handles.OPTIONS=OPTIONS;
...
set(handles.RemoveAl, 'Value', 0);
set(handles.RemoveNum, 'Value', 0);
set(handles.ParseSubd, 'Value', 0);
handles.update_struct_type='Files';
handles.removed_docs_type='Files';
handles.dictionary_type='Files';
handles.gwquery_type='Files';
...
if state==0,
    ...
    states=ones(28, 1);states([(3:10)])=0;
...
if state==0,
    ...
    set(handles.RadioQuery, 'Value', 0);
    set(handles.RadioDowndate, 'Value', 0);
    states=zeros(28, 1);states([(1:2) (7:8) 11 (26:28)])=1;
    handles=activate_uicontrol(states, handles);
end
guidata(hObject, handles);
...
if state==0,
    ...
    set(handles.RadioQuery, 'Value', 0);
    set(handles.RadioUpdate, 'Value', 0);
    states=zeros(28, 1);states([(7:10)])=1;
    handles=activate_uicontrol(states, handles);
end
guidata(hObject, handles);
...
if state==0,
    ...
    set(handles.RadioUpdate, 'Value', 0);
    set(handles.RadioDowndate, 'Value', 0);
    states=zeros(28, 1);states([(1:6) (11:13) 20 23 (26:28)])=1;
    handles=activate_uicontrol(states, handles);
end
guidata(hObject, handles);
